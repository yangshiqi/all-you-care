        ...
        <main className="container mx-auto px-4 py-8">
          ...
          <div className="mb-12 text-center">
            ...
            <div className="max-w-4xl mx-auto mt-8">
              ...
              {showTags && (
                <div className="vintage-border bg-card p-6 space-y-4 text-left">
                  {issueData.tagCategories.map((category, index) => (
                    <div key={index}>
          ...
          <div className="flex flex-col lg:flex-row gap-8 max-w-6xl mx-auto">
            ...
            <aside className="lg:w-64 flex-shrink-0">
              <div className="vintage-border bg-card p-4 sticky top-4">
                <button
                  className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider mb-4 w-full text-left"
                  onClick={() => setActiveSection(activeSection ? "" : "toc")}
                >
                  <ChevronDown className={`w-4 h-4 transition-transform ${activeSection === "toc" ? "rotate-180" : ""}`} />
                  {t('issueDetail.tableOfContents')}
                </button>
                <nav className="space-y-2">
                  {tableOfContents.map((item) => (
                    <a
                      key={item.id}
                      href={`#${item.id}`}
                      className="block text-sm hover:text-primary transition-colors py-1 px-2 hover:bg-background"
                      onClick={(e) => {
                        e.preventDefault();
