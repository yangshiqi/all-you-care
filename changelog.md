...
### 📋 项目规范更新
- **新增规范**: 在项目规范中添加了全面的组件和语法使用规范章节
- **核心要求**: 组件和语法必须使用相关库和框架的最新稳定版本，避免过时、废弃、不安全的使用
- **检查清单**: 扩展了代码提交前的检查清单，涵盖组件、语法、依赖包和安全等方面

### ✨ 规范内容
- **核心原则**: 
  - 使用最新版本：组件和语法必须使用相关库和框架的最新稳定版本
  - 避免过时用法：禁止使用已过时、废弃（deprecated）或不安全的语法和组件
  - 及时更新：当依赖包发布新版本时，及时检查并更新代码中的过时用法
  - 安全优先：优先选择安全、稳定、官方推荐的实现方式
  - 文档参考：在实现新功能前，查阅官方最新文档，确保使用推荐的语法和组件

- **React 组件规范**:
  - 使用 React 19 的最新特性和 API
  - 优先使用函数组件和 React Hooks，避免使用类组件（除非必要）
  - 优先使用 React Server Components（Next.js App Router）
  - 仅在需要交互时使用 `'use client'` 指令
  - 使用 ES6+ 模块语法（`import/export`），避免 CommonJS

- **TypeScript 语法规范**:
  - 严格使用 TypeScript 类型，避免使用 `any` 类型
  - 充分利用 TypeScript 的类型推断，减少冗余类型声明
  - 使用 TypeScript 5+ 的最新特性（如 `satisfies`、`const` 类型参数等）
  - 优先使用 `interface` 定义对象类型，使用 `type` 定义联合类型和工具类型

- **Next.js 特定规范**:
  - 使用 Next.js App Router（已弃用 Pages Router）
  - 使用 `src/proxy.ts` 而不是 `src/middleware.ts`（Next.js 已弃用 middleware.ts）
  - 使用 `await params` 处理动态路由参数（Next.js 16+）
  - 使用 `generateMetadata` 函数生成页面元数据
  - 使用 `generateStaticParams` 进行静态页面生成
  - 优先使用 Server Components 中的 `async/await` 进行数据获取

- **组件库规范**:
  - 使用最新版本的 shadcn/ui 组件
  - 定期运行 `npx shadcn@latest add` 更新组件到最新版本
  - 基于 shadcn/ui 组件进行扩展时，保持与最新版本兼容
  - 使用 Tailwind CSS 最新版本的类名和功能

- **依赖包管理**:
  - 定期运行 `npm outdated` 检查过时的依赖包
  - 定期运行 `npm audit` 检查安全漏洞
  - 主版本更新：谨慎评估，检查破坏性变更
  - 次版本更新：及时更新，通常向后兼容
  - 补丁版本更新：立即更新，修复 bug 和安全问题
  - 提交 `package-lock.json` 确保依赖版本一致性

- **安全检查**:
  - 使用 `npm audit` 或 `npm audit fix` 修复已知安全漏洞
  - 避免使用已停止维护的依赖包
  - 避免使用 `eval()` 和 `Function()` 构造函数
  - 使用参数化查询防止 SQL 注入（如 Supabase）
  - 验证和清理用户输入
  - 使用 HTTPS 和安全的认证机制

- **更新流程**:
  - 识别问题：通过控制台警告、文档或工具识别过时用法
  - 查阅文档：查阅官方最新文档和迁移指南
  - 制定计划：制定更新计划，评估影响范围
  - 逐步迁移：逐步更新代码，确保功能正常
  - 测试验证：充分测试更新后的代码
...
### 📝 修改文件
- `allaboutproject.md` - 扩展代码规范章节，添加全面的组件和语法使用规范

### ⚠️ 注意事项
- 所有开发人员在提交代码前必须检查是否使用了过时或废弃的组件和语法
- 如果发现过时用法，必须立即更新为最新推荐的方法
- 定期检查依赖包更新，及时迁移到新版本 API
- 优先考虑安全性和稳定性，避免使用不安全的实现方式

---

## 2025-12-16 - 优化首页订阅区域设计

### ✨ 设计优化
- **全新订阅卡片设计**: 参考主流 Newsletter 订阅设计（如 Morning Brew、TLDR、Superhuman）进行优化
- **清晰的视觉层次**: 添加醒目的顶部标题栏，带有邮件图标和小红点提醒
- **社会证明**: 添加模拟头像组和订阅人数统计，增加可信度
- **特色亮点徽章**: 使用彩色标签展示产品特点（免费、每日精选、AI精选、拒绝垃圾邮件）
- **现代化表单**: 优化输入框设计，添加邮件图标和圆角样式
- **CTA 按钮优化**: 添加箭头动画和 loading 状态
- **隐私提示**: 添加底部隐私保护提示，带有盾牌图标

### 🔧 技术实现
- 使用 Tailwind CSS 实现响应式设计
- 支持 dark mode（暗色模式）
- 添加微交互动画（hover、active、pulse 等）
- 保持与 Brevo 表单的兼容性

### 📝 修改文件
- `src/components/Hero.tsx` - 重构订阅区域 UI
- `src/lib/locales/en.ts` - 添加新的英文翻译文本
- `src/lib/locales/zh_CN.ts` - 添加新的中文翻译文本

### 🐛 Bug 修复
- 修复 `zh_CN.ts` 中的中文引号导致的解析错误（将 `""` 改为 `「」`）
