...
### 路由配置
- **首页**: `/[lang]` - 展示最新AI资讯和订阅功能（支持多语言）
- **期刊列表**: `/[lang]/issues` - 所有期刊的列表页面（支持分页，URL格式：`/[lang]/issues?page=N`）
- **期刊详情**: `/[lang]/issues/[slug]` - 动态期刊详情页（支持多语言）
- **演示页面**: `/issues-demo` - 期刊访问演示
- **测试页面**: `/test` - SSR功能测试
- **标签列表**: `/[lang]/tags` - 所有标签列表页面（包含标签使用统计，支持多语言）
- **标签页面**: `/[lang]/tags/[tag]` - 按标签筛选展示内容（支持SEO元数据，支持多语言）
- **订阅成功页**: `/[lang]/subscribe/success` - 订阅成功提示页面（支持激活状态，支持多语言）
- **SNOW订阅页**: `/[lang]/subscribe/snow` - SNOW新闻订阅页面（简洁设计风格，支持多语言）
...
#### React 组件规范
- **React 19**: 使用 React 19 的最新特性和 API
- **Hooks**: 优先使用函数组件和 React Hooks，避免使用类组件（除非必要）
- **Server Components**: 优先使用 React Server Components（Next.js App Router）
- **客户端组件**: 仅在需要交互时使用 `'use client'` 指令
- **组件导入**: 使用 ES6+ 模块语法（`import/export`），避免 CommonJS（`require/module.exports`）

#### TypeScript 语法规范
- **类型安全**: 严格使用 TypeScript 类型，避免使用 `any` 类型
- **类型推断**: 充分利用 TypeScript 的类型推断，减少冗余类型声明
- **最新语法**: 使用 TypeScript 5+ 的最新特性（如 `satisfies`、`const` 类型参数等）
- **接口定义**: 优先使用 `interface` 定义对象类型，使用 `type` 定义联合类型和工具类型

#### Next.js 特定规范
- **App Router**: 使用 Next.js App Router（已弃用 Pages Router）
...
- **数据获取**: 优先使用 Server Components 中的 `async/await` 进行数据获取

#### 组件库规范
- **shadcn/ui**: 使用最新版本的 shadcn/ui 组件
- **组件更新**: 定期运行 `npx shadcn@latest add` 更新组件到最新版本
- **自定义组件**: 基于 shadcn/ui 组件进行扩展时，保持与最新版本兼容
- **Tailwind CSS**: 使用 Tailwind CSS 最新版本的类名和功能

#### 依赖包管理
- **版本检查**: 定期运行 `npm outdated` 检查过时的依赖包
- **安全审计**: 定期运行 `npm audit` 检查安全漏洞
- **更新策略**: 
  - 主版本更新：谨慎评估，检查破坏性变更
  - 次版本更新：及时更新，通常向后兼容
  - 补丁版本更新：立即更新，修复 bug 和安全问题
- **锁定文件**: 提交 `package-lock.json` 确保依赖版本一致性

#### 安全检查
- **依赖安全**: 使用 `npm audit` 或 `npm audit fix` 修复已知安全漏洞
- **过时依赖**: 避免使用已停止维护的依赖包
- **安全最佳实践**: 
...
#### 更新流程
当发现过时或废弃的用法时：
1. **识别问题**: 通过控制台警告、文档或工具识别过时用法
2. **查阅文档**: 查阅官方最新文档和迁移指南
3. **制定计划**: 制定更新计划，评估影响范围
4. **逐步迁移**: 逐步更新代码，确保功能正常
5. **测试验证**: 充分测试更新后的代码
6. **更新文档**: 更新项目文档，记录变更
